...
export default function InsertPage() {
    useEffect(() => {
        ...
        script.src = 'https://lzpel.github.io/lambda360order/widget.js'; // Using widget.js from main branch or local if needed
        script.async = true;
    ...
    */}
            <div id="lambda360-order" data-model="https://lzpel.github.io/lambda360order/PA-001-DF7.json" style={{ width: '100%', height: '500px', border: '1px dashed #ccc', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#999' }}>
                (ここにスクリプトによって3Dビューワーが注入されます)
            </div>

            {/* 
        【想定される外部スクリプト】
        本来は以下ののようなタグを1行貼るだけで動作する形を目指します：
        <script src="https://lzpel.github.io/lambda360order/widget.js"></script>
        
        現在はデモとして、useEffectでスクリプトを読み込んでいます。
    */}
        </div>
    );
}
